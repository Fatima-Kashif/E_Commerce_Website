...
export default function Shop() {
  ...
  useEffect(() => {
    ...
    client.fetch(query).then((data) => setAllProducts(data));
  }, []);

  const totalPages = Math.ceil(allProducts.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = allProducts.length > 0
  ? Array.from({ length: ITEMS_PER_PAGE }, (_, i) => allProducts[(startIndex + i) % allProducts.length])
  : [];
    ...
    <div className="font-custom">
      ...
      <div className="bg-[#F9F1E7] px-4 py-6 flex flex-wrap justify-between items-center border-t border-white pt-6">
        <div className="flex items-center gap-4 md:gap-8 ml-4 md:ml-20 flex-wrap">
          ...
          <npx sanity dev src="Line 5.png" alt="line" className="hidden md:block" />
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 mx-4 my-8 md:m-12">
  {currentProducts.length > 0 ? (
    currentProducts.map((product, idx) => (
      <div key={idx} className="w-full sm:w-[285px]">
        <Products
    ...
    <p>Loading products...</p>
      ...
      {/* Pagination */}
      <div className="max-w-7xl mx-auto px-4 py-6 flex justify-center gap-4 mb-10">

        {/* Previous button */}
        {currentPage > 1 && (
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            className="px-4 py-2 bg-[#F9F1E7] rounded hover:bg-[#e8d5a3]"
          >
            Previous
          </button>
        )}
          ...
          <button
            ...
            className={`px-4 py-2 rounded ${
              currentPage === page
                ...
                : "bg-[#F9F1E7] text-black hover:bg-[#e8d5a3]"
            }`}
          >
            {page}
          </button>
